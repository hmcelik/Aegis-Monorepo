...
- ✅ Worker service operational with health checks
  **Suggested Tests:**
- _Unit:_ job id format, backoff calculation, DLQ routing.
- _Integration:_ publish → consume → ack; duplicate publish suppressed by jobId uniqueness.
- _E2E:_ 10k synthetic messages; ensure no loss/duplication; p99 processing < target.
  **Estimate:** 3d
...
- ✅ _Unit:_ idempotency key generation; state transitions (pending→sent→confirmed).
- ✅ _Integration:_ Action retry scenarios and failure handling validated.
- ✅ _E2E:_ Reprocess same message scenarios tested with deterministic behavior.
...
- ✅ _Unit:_ backoff schedule calculation; breaker state machine.
- ✅ _Integration:_ fake Telegram server returns 429 and 500; verify retries and pause/resume.
- ✅ _E2E:_ chaos test toggling failure patterns; no message loss; SLO respected.
  **Estimate:** 2d ← **COMPLETED**
...
- ✅ _Unit:_ deterministic hash mapping; rebalancing behavior.
- ✅ _Integration:_ mixed load with one hot chat; ensure other chats maintain throughput.
...
- ✅ Returns matches with spans for audit UI.
  **Suggested Tests:**
- _Unit:_ hit/miss tables; perf under 10k keywords.
- _Integration:_ combined with normalizer catches obfuscated spam.
  **Estimate:** 2d
  **Depends On:** AEG‑201.
...
- ✅ Multi-rule scoring: Combines multiple rule weights correctly
  **Suggested Tests:**
- _Unit:_ rule evaluation determinism.
- _Integration:_ pipeline routes to AI only when `review`.
- _E2E:_ A/B run on sample stream; LLM calls reduced ≥50% with stable precision.
  **Estimate:** 3d
  **Depends On:** AEG‑203.
...
- ⚠️ _Unit:_ budget arithmetic; edge at month rollover. (Blocked by dependencies)
- ⚠️ _Integration:_ worker reads cap; enforces degrade on exhaustion. (Blocked by dependencies)
- ⚠️ _E2E:_ simulate heavy traffic until cap; verify UI + behavior switch. (Blocked by dependencies)
...
- ✅ _Unit:_ hash stability; TTL expiry; cache metrics validation
- ✅ _Integration:_ repeated spam burst → AI call count reduced by ≥80%
...
- ✅ _Unit:_ token bucket refill; circuit breaker state transitions.
- ✅ _Integration:_ sustained load → proper throttling and backpressure.
- ✅ _E2E:_ AI service outage → graceful degradation without system failure.
...
- ✅ _Unit:_ schema validation; migrations dry‑run; repository operations
- ✅ _Integration:_ dual‑write consistency checks; feature flag transitions
- ✅ _E2E:_ cutover rehearsal scenarios; data integrity validation
...
- _Unit:_ aggregation math; date boundary handling
- _Integration:_ double‑run same day → single row stable; data consistency
  **Estimate:** 1d
  **Depends On:** AEG‑401 ✅
...
- Added to bot/api/worker with minimal config.
  **Suggested Tests:**
- _Unit:_ redaction rules.
- _Integration:_ trace spans emitted for a full message lifecycle.
  **Estimate:** 2d
  **Depends On:** none.
...
- Document SLOs (p99 TTA <1.5s; error budget 2%/30d).
  **Suggested Tests:**
- _Integration:_ dependency down → `/readyz` fails; liveness still true.
- _E2E:_ dashboards display metrics under load.
  **Estimate:** 1.5d
  **Depends On:** AEG‑501.
...
- System degrades gracefully (queue pause, retries, no crash); recovers automatically.
  **Suggested Tests:**
- _E2E:_ scripted chaos scenarios; assert no lost jobs; latency within degraded SLO.
  **Estimate:** 2d
...
- Zero side effects; uses same pipeline library as worker.
  **Suggested Tests:**
- _Integration:_ golden snapshots of simulator output for fixtures.
- _E2E:_ admin can tune thresholds and instantly see changed verdict.
  **Estimate:** 2d
  **Depends On:** AEG‑204.
...
- Edit budget/cap + degrade mode with optimistic UI and server validation.
  **Suggested Tests:**
- _Integration:_ API contract tests; unauthorized access denied.
- _E2E:_ change cap → worker behavior switches accordingly.
  **Estimate:** 1.5d
  **Depends On:** AEG‑301.
...
- Global incident banner configurable by super admin.
  **Suggested Tests:**
- _Integration:_ websocket/SSE updates reflect worker signals.
- _E2E:_ induce backlog → UI surfaces warning within 10s.
  **Estimate:** 1d
...
- Reject inbound management webhooks lacking valid signature/time window.
  **Suggested Tests:**
- _Unit:_ signature computation; skew handling.
- _Integration:_ replay attack attempt rejected.
  **Estimate:** 1d
  **Depends On:** none.
...
- Log redaction of usernames/ids where not needed.
  **Suggested Tests:**
- _Unit:_ redaction helpers.
- _Integration:_ retention job removes/compacts rows; idempotent.
  **Estimate:** 1.5d
  **Depends On:** AEG‑401.
...
- Security tests cover IDOR attempts.
  **Suggested Tests:**
- _Integration:_ forbidden cross‑tenant access returns 403.
  **Estimate:** 1d
  **Depends On:** none.
...
- `pnpm e2e` runs the suite headless in CI.
  **Suggested Tests:**
- _E2E:_ green pipeline proves basic flows.
  **Estimate:** 1d
  **Depends On:** EP1, EP4.
...
- Versioned contracts fail CI on breaking changes.
  **Suggested Tests:**
- _Contract:_ enforce OpenAPI adherence.
  **Estimate:** 1d
  **Depends On:** AEG‑501.
...
- UI surfaces experimental features badge.
  **Suggested Tests:**
- _Unit:_ flag resolution precedence.
- _Integration:_ turn on/off without redeploy.
  **Estimate:** 1d
  **Depends On:** none.
...
- Written runbook for canarying workers; metrics to watch; how to rollback.
  **Suggested Tests:**
- _Doc review:_ peer‑approved; dry‑run in staging.
  **Estimate:** 0.5d
  **Depends On:** none.
...
- Linting/enforced strictness for new code in packages/normalizer/policy.
  **Suggested Tests:**
- _Build:_ CI compiles TS; type errors gate merges.
  **Estimate:** 1d
  **Depends On:** none.
...
- `"strict": true`; exported types consumed by apps.
  **Suggested Tests:**
- _Build:_ no `any` leakage across package boundaries.
  **Estimate:** 1d
  **Depends On:** AEG‑901.
