  ...
  async performHealthCheck() {
    try {
      await databaseBreaker.execute(async () => {
        await withTimeout(this.db.get('SELECT 1 as health_check'), 5000, 'Database health check');
      });
...
export const getAllGroups = async () => {
  const result = await safeDbOperation('SELECT * FROM groups', [], 'get all groups');
...
export const recordStrike = async (chatId, userId, logData) => {
  ...
  const result = await safeDbExecution(async () => {
    ...
    const result = await db.get(
      'SELECT count FROM strikes WHERE chatId = ? AND userId = ?',
      validatedChatId,
      validatedUserId
    );
...
export const removeStrike = async (chatId, userId, amount) => {
  ...
  const result = await safeDbExecution(async () => {
    ...
    // Get current count
    const currentStrikes = await db.get(
      'SELECT count FROM strikes WHERE chatId = ? AND userId = ?',
      validatedChatId,
      validatedUserId
    );
    ...
    // Update strikes
    await db.run(
      'UPDATE strikes SET count = ?, timestamp = CASE WHEN ? > 0 THEN ? ELSE NULL END WHERE chatId = ? AND userId = ?',
      newCount,
      newCount,
...
export const getTotalDeletionsToday = async chatId => {
  return safeDbExecution(async () => {
    ...
    const result = await dbConnManager
      .getConnection()
      .get(
...
export const getUser = async userId => {
  return safeDbExecution(async () => {
    ...
    return await dbConnManager
      .getConnection()
      .get('SELECT * FROM users WHERE userId = ?', validatedUserId);
  }, 'getUser');
};
...
export const isUserGroupAdmin = async (userId, groupId) => {
  ...
  try {
    ...
    console.warn(
      `Could not check admin status for user ${userId} in group ${groupId}:`,
      error.message
    );
    return false;
  }
  ...
  try {
    ...
    for (const keyword of keywords) {
      await dbInstance.run(
        'INSERT INTO keyword_whitelist (chatId, keyword) VALUES (?, ?)',
        chatId,
        keyword
      );
    }
  ...
  try {
    ...
    const scannedMessagesResult = await safeDbOperation(
      `
        ...
        `,
      [validatedGroupId, startDate.toISOString(), endDate.toISOString()],
      'get scanned messages count'
    );

    const scannedMessages =
      scannedMessagesResult && scannedMessagesResult.length > 0
    ...
    const legacyAutoEntriesResult = await safeDbOperation(
      `
        ...
        `,
      [validatedGroupId, startDate.toISOString(), endDate.toISOString()],
      'get legacy auto entries'
    );

    const spamCount =
      (spamMessagesResult && spamMessagesResult.length > 0 ? spamMessagesResult[0].count || 0 : 0) +
      (legacyAutoEntriesResult && legacyAutoEntriesResult.length > 0
        ? legacyAutoEntriesResult[0].count || 0
        : 0);
    const profanityCount =
      profanityMessagesResult && profanityMessagesResult.length > 0
        ? profanityMessagesResult[0].count || 0
        : 0;
    ...
    const deletedMessagesLegacyResult = await safeDbOperation(
      `
        ...
        `,
      [validatedGroupId, startDate.toISOString(), endDate.toISOString()],
      'get deleted messages legacy'
    );

    const totalDeleted =
      (deletedMessagesNewResult && deletedMessagesNewResult.length > 0
        ? deletedMessagesNewResult[0].count || 0
        : 0) +
      (deletedMessagesLegacyResult && deletedMessagesLegacyResult.length > 0
        ? deletedMessagesLegacyResult[0].count || 0
        : 0);
    ...
    return {
      ...
      mutedUsers:
        mutedUsersResult && mutedUsersResult.length > 0 ? mutedUsersResult[0].count || 0 : 0,
      kickedUsers:
      ...
      flaggedRate:
        totalScanned > 0
          ? Math.round(((spamCount + profanityCount) / totalScanned) * 10000) / 100
          : 0,
      autoModerationEfficiency: {
        ...
        usersActioned:
          (mutedUsersResult && mutedUsersResult.length > 0 ? mutedUsersResult[0].count || 0 : 0) +
          (kickedUsersResult && kickedUsersResult.length > 0
            ? kickedUsersResult[0].count || 0
            : 0) +
          (bannedUsersResult && bannedUsersResult.length > 0 ? bannedUsersResult[0].count || 0 : 0),
      },
    };
  ...
  try {
    ...
    const avgResponseTime =
      responseTimeStats.length > 0
        ? responseTimeStats.reduce((sum, row) => sum + row.response_time_seconds, 0) /
    ...
    return {
      ...
      effectivenessScore:
        responseTimeStats.length > 0 ? Math.max(0, 100 - (avgResponseTime / 60) * 10) : 0, // Score based on response time
    };
  } catch (error) {
