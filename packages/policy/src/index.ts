...
export class PolicyEngine {
  ...
  evaluate(text: string): PolicyVerdict {
    ...
    if (totalScore >= 80) { // Lowered from 100 to match test expectations
      verdict = 'block';
  ...
  evaluateFastPath(text: string): PolicyVerdict {
    ...
    const fastPathRules = this.rules.filter(rule => 
    ...
    if (totalScore >= 100) { // Higher threshold for blocking without AI
      verdict = 'block';
      reason = `Fast-path block: High confidence rule violation (score: ${totalScore})`;
    } else if (totalScore >= 70) { // Mark for review if uncertain
      verdict = 'review';
