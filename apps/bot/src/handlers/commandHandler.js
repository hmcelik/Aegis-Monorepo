...
import {
  ...
  getChatMember,
  sendDocument,
} from '@telegram-moderator/shared/src/services/telegram.js';
...
if (!SUPER_ADMIN_USER_ID) {
  logger.warn(
    'ADMIN_USER_ID not set in environment variables. Super admin commands will be disabled.'
  );
} else {
...
const handleCommandError = async (originalMessage, errorText) => {
  try {
    ...
    const errorMsg = await sendMessage(originalMessage.chat.id, escapedErrorText, {
...
${
  SUPER_ADMIN_USER_ID && from.id.toString() === SUPER_ADMIN_USER_ID
...
`
    ...
    case '/clearcache':
      logger.info(
        `Super admin command attempted in private chat: ${command} by user ${from.id} (configured admin: ${SUPER_ADMIN_USER_ID})`
      );
...
const handleGroupCommand = async msg => {
  ...
  if (superAdminCommands.includes(command)) {
    logger.info(
      `Super admin command attempted: ${command} by user ${from.id} (configured admin: ${SUPER_ADMIN_USER_ID})`
    );
...
const handlePublicGroupCommand = async (msg, isAdmin) => {
  ...
  if (command === '/mystrikes') {
    try {
      ...
      if (history.length > 0) {
        ...
        for (const record of history) {
          ...
          if (actionType.startsWith('MANUAL')) {
            ...
            if (logData.type === 'MANUAL-STRIKE-ADD')
              actionDetail = `Added ${logData.amount} strike(s)`;
            else if (logData.type === 'MANUAL-STRIKE-REMOVE')
              actionDetail = `Removed ${logData.amount} strike(s)`;
            else if (logData.type === 'MANUAL-STRIKE-SET')
              actionDetail = `Set strikes to ${logData.amount}`;
      ...
      const confirmationMsg = await sendMessage(
        chat.id,
        `I've sent your strike report to you in a private message, ${from.first_name}.`
      );
      ...
      if (error.response?.body.description.includes("bot can't initiate conversation")) {
        const errorMsg = await sendMessage(
          chat.id,
          `${from.first_name}, I can't send you a private message. Please start a chat with me first!`
        );
    ...
    try {
      ...
      const confirmationMsg = await sendMessage(
        chat.id,
        `I've sent you the command list in a private message, ${from.first_name}.`
      );
      setTimeout(() => deleteMessage(chat.id, confirmationMsg.message_id), 3000);
    } catch (error) {
      if (error.response?.body.description.includes("bot can't initiate conversation")) {
  ...
  switch (command) {
    case '/register':
      ...
      if (existingGroup) {
        ...
        await sendMessage(
          chat.id,
          `✅ This group, "${chat.title}", has been successfully registered. Admins can now manage me via a private message.`
        );
      }
      break;
    ...
    case '/auditlog':
      try {
        ...
        if (logs.length === 0) {
          await sendMessage(
            from.id,
            `There are no audit log entries for the group "${chat.title}".`
          );
        } else {
          ...
          await sendDocument(
            ...
            logBuffer,
            {},
            { filename: `audit_log_${chat.title}.txt`, contentType: 'text/plain' }
          );
        }
        const confirmationMsg = await sendMessage(
          chat.id,
    ...
    default: // Strike commands
      if (!target) {
        return handleCommandError(
          msg,
          `Invalid usage. Please specify a user. Use '/help' for details.`
        );
      }
      ...
      if (!user) {
        return handleCommandError(
          msg,
          `User ${target} not found. They may need to send a message so I can see them.`
        );
      }
      ...
      if (command === '/checkstrikes') {
        try {
          ...
          if (history.length > 0) {
            ...
            for (const record of history) {
              report += `\n─────────────────────\n`;
              const logData = JSON.parse(record.logData);
              const timestamp = new Date(record.timestamp).toLocaleString('en-GB', {
                timeZone: 'UTC',
              });
              const actionType = logData.type || 'AUTO';
              if (actionType.startsWith('MANUAL')) {
          ...
          const confirmationMsg = await sendMessage(
            chat.id,
            `I've sent ${target}'s strike report to you privately.`
          );
          setTimeout(() => {
            deleteMessage(chat.id, msg.message_id).catch(() => {});
            deleteMessage(chat.id, confirmationMsg.message_id).catch(() => {});
        ...
        // addstrike, removestrike, setstrike
        ...
        if (command === '/addstrike') {
          if (isNaN(amount) || amount <= 0) {
            return handleCommandError(
              msg,
              `Invalid usage. Use \`/addstrike @user <amount>\`. Use \`/help\` for details.`
            );
          }
          const newCount = await db.addStrikes(chat.id.toString(), user.userId, amount);
          await db.logManualAction(chat.id.toString(), user.userId, {
          ...
          await sendMessage(
            chat.id,
            `Strikes for ${target} changed from ${beforeStrikes.count} -> ${newCount} (+${amount}).`
          );
          ...
          await sendMessage(
            chat.id,
            `Strikes for ${target} changed from ${beforeStrikes.count} -> ${newCount} (-${amountToRemove}).`
          );
        } else if (command === '/setstrike') {
          if (isNaN(amount) || amount < 0) {
            return handleCommandError(
              msg,
              `Invalid usage. Use \`/setstrike @user <amount>\`. Use \`/help\` for details.`
            );
          }
          ...
          await sendMessage(
            chat.id,
            `Strikes for ${target} set from ${beforeStrikes.count} -> ${amount}.`
          );
        }
        await deleteMessage(chat.id, msg.message_id);
      }
    ...
    if (userGroups.length === 1) {
      ...
      if (nextAction === 'mystrikes') {
        ...
        if (nextAction === 'status') {
          ...
          const sentMenuMessage = await sendMessage(chatId, text, {
            ...keyboard,
            parse_mode: 'Markdown',
          });
  ...
  switch (command) {
    ...
    case '/broadcast':
      if (args.length === 0) {
        await sendMessage(
          chat.id,
          '📢 **Broadcast Command**\n\nUsage: `/broadcast <message>`\n\nThis will send a message to all registered groups.',
          { parse_mode: 'Markdown' }
        );
        return;
      }
