...
export const handleMessage = async msg => {
    ...
    if (strikes.count > 0 && strikes.timestamp) {
      ...
      if (diffDays > groupSettings.goodBehaviorDays) {
        ...
        try {
          await sendMessage(
            from.id,
            'Your good behavior has been noticed, and one of your strikes in ' +
              chat.title +
              ' has been removed. Keep it up!'
          );
        } catch (error) {
  ...
  if (
    groupSettings.keywordWhitelistBypass &&
    groupSettings.whitelistedKeywords.some(kw => text.toLowerCase().includes(kw.toLowerCase()))
  ) {
    logger.info(
      `Ignoring message from ${from.id} in chat ${chat.id} due to whitelisted keyword bypass.`
    );
    return;
  }
  ...
  try {
    ...
    const shouldCheckProfanity =
      groupSettings.profanityEnabled && groupSettings.profanityThreshold > 0;
    ...
    logger.debug(
      `Message analysis - Spam: ${spamResult.score.toFixed(2)}, Profanity: ${profanityResult.severity.toFixed(2)}`
    );
    ...
    const isProfanityViolation =
      shouldCheckProfanity && profanityResult.severity >= groupSettings.profanityThreshold;
    ...
    if (isSpamViolation || isProfanityViolation) {
      ...
      await db.logManualAction(chat.id.toString(), from.id.toString(), {
        ...
        messageLength: text.length,
        thresholdExceeded: isSpamViolation
          ? groupSettings.spamThreshold
          : groupSettings.profanityThreshold,
      });
      ...
      logger.info(
        `User ${from.id} in chat ${chat.id} committed ${violationType} strike #${newStrikeCount}.`
      );
    ...
    logger.error(`Error processing message from ${from.id} in chat ${chat.id}: ${error.message}`, {
      stack: error.stack,
    });
  }
};
...
async function applyPenalty(chatId, user, strikeCount, settings, logData) {
      ...
      execute: async () => {
        ...
        const warningMessage =
          violationType === 'PROFANITY'
            ? settings.profanityWarningMessage
        ...
        const reason = logData
          ? ` for ${violationEmoji} ${violationType.toLowerCase()}: "*${escapeMarkdownV2(logData.messageExcerpt)}*"`
          : '';
        ...
        if (settings.warningMessageDeleteSeconds > 0) {
          setTimeout(
            () => deleteMessage(chatId, sentMsg.message_id),
            settings.warningMessageDeleteSeconds * 1000
          );
        }
  ...
  const triggeredActions = actions.filter(
    action => action.level > 0 && strikeCount >= action.level
  );
  ...
  const actionToExecute = triggeredActions.reduce((prev, current) =>
    prev.level > current.level ? prev : current
  );

  logger.warn(
    `Executing penalty: ${actionToExecute.name} for user ${user.id} in chat ${chatId} at strike #${strikeCount}.`
  );
