...
export const handleCallback = async callbackQuery => {
  ...
  if (action !== 'select_group' && !targetChatId) {
    logger.warn(
      `Callback handler invoked without a targetChatId for user ${from.id}. Data: ${data}`
    );
    await telegram.answerCallbackQuery(callbackQuery.id, {
      text: 'Your session may have expired. Please use /settings again.',
    });
    return;
  }
  ...
  try {
    ...
    if (action === 'select_group') {
      ...
      if (nextAction === 'status') {
        ...
        await telegram.editMessageText(response, {
        ...
        await telegram.editMessageText(report, {
          chat_id: message.chat.id,
          message_id: message.message_id,
      ...
      switch (action) {
        ...
        case 'list_mods':
          ...
          const items = isKeywords
            ? await db.getWhitelistKeywords(targetChatId)
            : (await getGroupSettings(targetChatId)).moderatorIds;
        ...
        case 'toggle_profanity':
          ...
          await telegram.answerCallbackQuery(callbackQuery.id, {
            text: `Profanity Filter is now ${newProfanityValue ? 'ON' : 'OFF'}`,
          });
...
bot.on('text', async msg => {
  ...
  try {
    ...
    if (action.startsWith('set_')) {
      ...
      switch (action) {
        ...
        case 'set_mute_duration':
          ...
          value = result.value;
          if (!result.valid)
            responseMessage = `❌ Invalid value. Duration must be a positive number.`;
          break;
        ...
        case 'set_good_behavior':
          result = handleNumericInput(text, true);
          settingKey =
            action === 'set_strike_expiration' ? 'strikeExpirationDays' : 'goodBehaviorDays';
          value = result.value;
        ...
        default: // Penalty Levels
          ...
          value = result.value;
          if (!result.valid)
            responseMessage = `❌ Invalid value. Strike level must be a positive number.`;
          break;
      }
    ...
    if (['set_threshold', 'toggle_bypass', 'set_spam_warning'].includes(action)) {
      ...
      ['set_profanity_threshold', 'set_profanity_warning', 'toggle_profanity'].includes(action)
    ) {
      ...
      [
        'set_mute_duration',
        'set_warning_delete_seconds',
        'set_strike_expiration',
        'set_good_behavior',
      ].includes(action)
    ) {
    ...
    const newMenuMessage = await telegram.sendMessage(chat.id, menuText, {
      ...keyboard,
      parse_mode: 'Markdown',
    });
