...
export const aiSensitivityKeyboard = (settings, chatId) => ({
  reply_markup: {
    inline_keyboard: [
      ...
      [
        {
          text: `🎯 Threshold (current: ${(settings.spamThreshold * 100).toFixed(0)}%)`,
          callback_data: `set_threshold:${chatId}`,
        },
      ],
      // Button to toggle the keyword bypass feature.
      [
        {
          text: `${settings.keywordWhitelistBypass ? '✅' : '❌'} Keyword Bypass (${settings.keywordWhitelistBypass ? 'ON' : 'OFF'})`,
          callback_data: `toggle_bypass:${chatId}`,
        },
      ],
