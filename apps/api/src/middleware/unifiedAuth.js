...
export const unifiedAuth = (req, res, next) => {
  try {
    ...
    if (initData) {
      ...
      if (!botToken) {
        return next(
          ApiError.fromType(ERROR_TYPES.TELEGRAM_AUTH_FAILED, 'Bot token not configured')
        );
      }

      if (!validateTelegramWebAppData(initData, botToken)) {
        return next(
          ApiError.fromType(ERROR_TYPES.TELEGRAM_AUTH_FAILED, 'Invalid Telegram WebApp data')
        );
      }
      ...
      try {
        ...
        if (!telegramUser) {
          return next(
            ApiError.fromType(ERROR_TYPES.TELEGRAM_AUTH_FAILED, 'No user data found in initData')
          );
        }
        ...
        return next(
          ApiError.fromType(ERROR_TYPES.TELEGRAM_AUTH_FAILED, 'Failed to parse Telegram user data')
        );
      }
    }
    ...
    return next(
      ApiError.fromType(
        ...
        'No authentication method provided (JWT token or Telegram WebApp data required)'
      )
    );
  } catch (error) {
    ...
    return next(
      ApiError.fromType(ERROR_TYPES.HTTP_500, 'Authentication middleware error: ' + error.message)
    );
  }
};
