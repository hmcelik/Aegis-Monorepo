...
export class ShardManager {
  ...
  private initializeShards(): void {
    ...
    const concurrencyPerShard = Math.max(
      1,
      Math.floor(this.config.concurrency / this.config.partitionCount)
    );
  ...
  async getMetrics(): Promise<ShardingMetrics> {
    ...
    const variance =
      throughputs.reduce((sum, tp) => sum + Math.pow(tp - avgThroughput, 2), 0) /
      throughputs.length;
