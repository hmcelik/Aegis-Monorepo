        ...
        {dashboardData?.groups?.length > 0 ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            {dashboardData.groups.map(group => (
              <div
                key={group.id}
                className="group relative bg-gradient-to-br from-slate-50 to-white border border-slate-200 rounded-2xl p-6 hover:shadow-md hover:border-slate-300 transition-all duration-300"
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex items-center">
                    ...
                    >
                      <div
                        className={`w-2 h-2 rounded-full ${group.bot_active ? 'bg-green-400' : 'bg-red-400'}`}
                      ></div>
          ...
          <div className="text-center py-16 bg-gradient-to-br from-slate-50 to-white border border-slate-200 rounded-2xl">
            ...
            <p className="text-slate-600 mb-6">Connect via Telegram Mini App to see your groups</p>
            <button
              onClick={onReloadData}
