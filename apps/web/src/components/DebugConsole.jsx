...
const DebugConsole = ({ isVisible, onToggle }) => {
  ...
  const consoleUI = (
    ...
    >
      ...
      >
        <span style={{ fontWeight: 'bold' }}>🔍 Debug Console ({logs.length} logs)</span>
        <div style={{ display: 'flex', gap: '8px' }}>
          <button onClick={() => setIsMinimized(!isMinimized)} style={btnStyle('#00ff00')}>
            {isMinimized ? '⬆️' : '⬇️'}
          </button>
          <button onClick={clearLogs} style={btnStyle('#00ff00')}>
            🗑️
          </button>
          <button onClick={onToggle} style={btnStyle('#ff4444')}>
            ✖️
          </button>
        </div>
      </div>
