...
const GroupStats = ({ stats = {}, loading, onRefresh, onPeriodChange }) => {
  ...
  const isLegacyJWTFormat =
    safeStats.totalMessagesProcessed !== undefined || safeStats.violationsDetected !== undefined;
  ...
  const actions =
    (safeStats?.penalties?.mutedUsers || 0) +
    (safeStats?.penalties?.kickedUsers || 0) +
    (safeStats?.penalties?.bannedUsers || 0);

  const violationRate = totalMessages > 0 ? ((violations / totalMessages) * 100).toFixed(1) : '0';

  const actionEfficiency = violations > 0 ? ((actions / violations) * 100).toFixed(1) : '0';
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 p-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {statItems.map(item => (
          <div
            key={item.key}
            className="group relative bg-gradient-to-br from-white to-slate-50 border border-slate-200 rounded-2xl p-6 hover:shadow-md hover:border-slate-300 transition-all duration-300"
          >
        ...
        <div className="mb-8">
          ...
          <div className="space-y-3">
            {safeStats.topViolationTypes.map((violation, index) => (
              <div
                key={violation.type}
                className="flex items-center justify-between bg-gradient-to-r from-slate-50 to-white rounded-2xl p-4 border border-slate-200 hover:shadow-sm transition-all duration-300"
              >
                <div className="flex items-center gap-4">
                  <span className="bg-blue-100 text-blue-700 text-sm font-bold px-3 py-1 rounded-full min-w-[2.5rem] text-center">
                    #{index + 1}
                  </span>
            ...
            {Object.entries(safeStats)
              ...
              .map(([key, value]) => {
                ...
                return (
                  <div
                    key={key}
                    className="bg-gradient-to-br from-slate-50 to-white rounded-xl p-6 border border-slate-200 hover:shadow-sm transition-all duration-200"
                  >
                    <div className="flex items-start gap-3">
                      <div className="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center flex-shrink-0">
                        <span className="text-lg">{getIconForKey(key)}</span>
                      </div>
