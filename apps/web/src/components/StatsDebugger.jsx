...
const StatsDebugger = () => {
  ...
  const testStats = async () => {
    ...
    try {
      ...
      } else if (groupsResponse?.data?.data && Array.isArray(groupsResponse.data.data) && groupsResponse.data.data.length > 0) {
        // Unified API format: {success, data: [groups]}
        groupId = groupsResponse.data.data[0].id;
      } else if (groupsResponse?.data?.groups && Array.isArray(groupsResponse.data.groups) && groupsResponse.data.groups.length > 0) {
      ...
      if (!groupId) {
        ...
        setTestResults(prev => prev + `Groups response: ${JSON.stringify(groupsResponse, null, 2)}\n`);
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        setTestResults(prev => prev + `Audit logs response:\n${JSON.stringify(auditResponse, null, 2)}\n`);
        
        if (auditResponse?.data) {
          setTestResults(prev => prev + `Total audit entries found: ${auditResponse.data.length}\n`);
          if (auditResponse.data.length > 0) {
            setTestResults(prev => prev + `Sample audit entry:\n${JSON.stringify(auditResponse.data[0], null, 2)}\n`);
          }
        }
      } catch (auditError) {
      ...
      for (const period of periods) {
        try {
          ...
          setTestResults(prev => prev + `WebApp Stats (${period}) Success:\n${JSON.stringify(webAppStats?.data?.data?.stats, null, 2)}\n`);
        } catch (webAppError) {
      ...
      for (const period of periods) {
        try {
          ...
          setTestResults(prev => prev + `JWT Stats (${period}) Success:\n${JSON.stringify(jwtStats?.data, null, 2)}\n`);
        } catch (jwtError) {
      ...
      try {
        ...
        const token = localStorage.getItem('telegram_auth_token') || localStorage.getItem('authToken');
        ...
        if (initData) {
          ...
          if (response.ok) {
            ...
            setTestResults(prev => prev + `Raw WebApp API Response:\n${JSON.stringify(data, null, 2)}\n`);
          } else {
            const errorText = await response.text();
            setTestResults(prev => prev + `Raw WebApp API Error (${response.status}): ${errorText}\n`);
          }
        }
        ...
        if (token) {
          ...
          if (response.ok) {
            const data = await response.json();
            setTestResults(prev => prev + `Raw JWT API Response:\n${JSON.stringify(data, null, 2)}\n`);
          } else {
            const errorText = await response.text();
