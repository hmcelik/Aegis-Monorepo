...
vi.mock('apps/api/src/middleware/checkGroupAdmin.js', () => ({
  ...
  describe('GET /api/v1/groups/:groupId/audit', () => {
    it('should get paginated audit log entries', async () => {
      ...
      db.getDb.mockReturnValue({
        get: vi.fn().mockResolvedValueOnce(mockTotalResult).mockResolvedValueOnce(mockTotalResult),
    ...
    it('should filter by user ID', async () => {
      ...
      await request(app).get('/api/v1/groups/test-group/audit?userId=specific-user').expect(200);
    ...
    it('should filter by type AUTO', async () => {
      ...
      await request(app).get('/api/v1/groups/test-group/audit?type=AUTO').expect(200);

      // Check that the query filters for AUTO type
      expect(mockDbGet).toHaveBeenCalledWith(
        expect.stringContaining(
          'JSON_EXTRACT(logData, "$.type") IS NULL OR JSON_EXTRACT(logData, "$.type") NOT LIKE "MANUAL%"'
        ),
    ...
    it('should filter by manual strike type', async () => {
      ...
      await request(app).get('/api/v1/groups/test-group/audit?type=MANUAL-STRIKE-ADD').expect(200);
  ...
  describe('GET /api/v1/groups/:groupId/audit/export', () => {
    ...
    it('should export audit log as CSV', async () => {
      ...
      expect(response.headers['content-disposition']).toMatch(
        /attachment; filename="audit_log_test-group_/
      );

      const csvContent = response.text;
      expect(csvContent).toContain('ID,Timestamp,Chat ID,User ID,Type,Action');
      expect(csvContent).toContain(
        '1,2023-01-01T00:00:00.000Z,test-group,user1,MANUAL-STRIKE-ADD,Added 2 strike(s)'
      );
