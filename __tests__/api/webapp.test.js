...
describe('WebApp API Endpoints', () => {
  ...
  describe('GET /api/v1/webapp/health', () => {
    it('should return health status', async () => {
      const response = await request(app).get('/api/v1/webapp/health').expect(200);
  ...
  describe('POST /api/v1/webapp/auth', () => {
    it('should reject request without Telegram initData', async () => {
      const response = await request(app).post('/api/v1/webapp/auth').expect(401);
  ...
  describe('GET /api/v1/webapp/user/profile', () => {
    it('should require Telegram WebApp authentication', async () => {
      const response = await request(app).get('/api/v1/webapp/user/profile').expect(401);
  ...
  describe('GET /api/v1/webapp/user/groups', () => {
    it('should require Telegram WebApp authentication', async () => {
      const response = await request(app).get('/api/v1/webapp/user/groups').expect(401);
  ...
  describe('GET /api/v1/webapp/group/:groupId/settings', () => {
    it('should require Telegram WebApp authentication', async () => {
      const response = await request(app).get('/api/v1/webapp/group/test123/settings').expect(401);
  ...
  describe('GET /api/v1/webapp/group/:groupId/stats', () => {
    it('should require Telegram WebApp authentication', async () => {
      const response = await request(app).get('/api/v1/webapp/group/test123/stats').expect(401);
  ...
  describe('CORS Headers', () => {
    ...
    it('should handle OPTIONS requests', async () => {
      const response = await request(app).options('/api/v1/webapp/health').expect(200); // CORS is configured with optionsSuccessStatus: 200
    });
  });
  ...
  describe('Rate Limiting', () => {
    it('should apply rate limiting to API endpoints', async () => {
      ...
      const requests = [];
      for (let i = 0; i < 5; i++) {
        requests.push(request(app).get('/api/v1/webapp/health').expect(200));
      }
