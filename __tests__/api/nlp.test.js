...
describe('NLP API Endpoints', () => {
  ...
  describe('Authentication', () => {
    it('should require authentication for all endpoints', async () => {
      await request(app).get('/api/v1/nlp/status').expect(401);

      await request(app).post('/api/v1/nlp/test/spam').send({ text: 'test' }).expect(401);

      await request(app).post('/api/v1/nlp/test/profanity').send({ text: 'test' }).expect(401);

      await request(app).post('/api/v1/nlp/analyze').send({ text: 'test' }).expect(401);
    });
  });
});
