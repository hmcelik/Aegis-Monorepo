...
import { handleCommand } from 'apps/bot/src/handlers/commandHandler.js';
...
describe('Command Handler', () => {
  ...
  describe('Group Commands', () => {
    ...
    describe('/mystrikes', () => {
      test('should send a user their strike history privately', async () => {
        ...
        expect(telegram.sendMessage).toHaveBeenCalledWith(
          USER_ID,
          expect.stringContaining('Your Strike Report'),
          expect.any(Object)
        );
        expect(telegram.sendMessage).toHaveBeenCalledWith(
          CHAT_ID,
          `I've sent your strike report to you in a private message, ${mockUser.first_name}.`
        );
      });
    });
    ...
    describe('Permissions and Error Handling', () => {
      ...
      test('should show error if target user is not found', async () => {
        ...
        expect(telegram.sendMessage).toHaveBeenCalledWith(
          CHAT_ID,
          expect.stringContaining('User @unknownuser not found'),
          expect.any(Object)
        );
      });
      ...
      test('should show error for invalid amount in /addstrike', async () => {
        ...
        expect(telegram.sendMessage).toHaveBeenCalledWith(
          CHAT_ID,
          expect.stringContaining('Invalid usage'),
          expect.any(Object)
        );
      });
    });
    ...
    describe('Strike Management Commands', () => {
      ...
      test('/addstrike should add strikes and log the action', async () => {
        const msg = {
          from: mockAdmin,
          chat: mockChat,
          text: '/addstrike @targetuser 2 Manual strike',
          message_id: 1,
        };
        ...
        expect(db.logManualAction).toHaveBeenCalledWith(
          CHAT_ID,
          TARGET_USER_ID,
          expect.objectContaining({ type: 'MANUAL-STRIKE-ADD', amount: 2 })
        );
        expect(telegram.sendMessage).toHaveBeenCalledWith(
          CHAT_ID,
          'Strikes for @targetuser changed from 3 -> 5 (+2).'
        );
      });
      ...
      test('/removestrike should remove strikes and log the action', async () => {
        const msg = {
          from: mockAdmin,
          chat: mockChat,
          text: '/removestrike @targetuser 1',
          message_id: 1,
        };
        ...
        expect(db.logManualAction).toHaveBeenCalledWith(
          CHAT_ID,
          TARGET_USER_ID,
          expect.objectContaining({ type: 'MANUAL-STRIKE-REMOVE', amount: 1 })
        );
        expect(telegram.sendMessage).toHaveBeenCalledWith(
          CHAT_ID,
          'Strikes for @targetuser changed from 3 -> 2 (-1).'
        );
      });
      ...
      test('/setstrike should set strikes and log the action', async () => {
        ...
        expect(db.logManualAction).toHaveBeenCalledWith(
          CHAT_ID,
          TARGET_USER_ID,
          expect.objectContaining({ type: 'MANUAL-STRIKE-SET', amount: 0 })
        );
        expect(telegram.sendMessage).toHaveBeenCalledWith(
          CHAT_ID,
          'Strikes for @targetuser set from 3 -> 0.'
        );
      });
    });
  });
  ...
  describe('Private Chat Commands', () => {
    test('/start should send a welcome message', async () => {
      ...
      expect(telegram.sendMessage).toHaveBeenCalledWith(
        USER_ID,
        expect.stringContaining('Hello\\! I am the AI Moderator Bot'),
        expect.any(Object)
      );
    });
    ...
    test('/mystrikes should trigger group selection for a user in multiple groups', async () => {
      ...
      expect(telegram.sendMessage).toHaveBeenCalledWith(
        USER_ID,
        'Please choose a group for the /mystrikes command:',
        expect.any(Object)
      );
    });
