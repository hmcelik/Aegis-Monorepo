...
import * as nlp from '@telegram-moderator/shared/src/services/nlp.js';
...
vi.mock('@telegram-moderator/shared/src/services/telegram.js');
...
describe('Message Handler', () => {
  ...
  test('should take action on promotional messages', async () => {
    ...
    expect(db.recordStrike).toHaveBeenCalledWith(
      mockMsg.chat.id.toString(),
      mockMsg.from.id.toString(),
      expect.any(Object)
    );
  ...
  describe('Good Behavior Forgiveness', () => {
    test('should remove one strike for good behavior', async () => {
      ...
      expect(db.removeStrike).toHaveBeenCalledWith(
        mockMsg.chat.id.toString(),
        mockMsg.from.id.toString(),
        1
      );
      expect(telegram.sendMessage).toHaveBeenCalledWith(
        mockMsg.from.id,
        expect.stringContaining('Your good behavior has been noticed')
      );
    });
